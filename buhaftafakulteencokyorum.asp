...
<title>Bu Hafta En &Ccedil;ok Yorumlananlar - Gazitiraf.net</title>
      ...
      <tr>
        ...
        <td width="505" valign="top">
		  ...
		  <table width="512" height="19" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td><span class="yazi style3"><strong>Bu Hafta En &Ccedil;ok Yorumlananlar</strong></span> <img src="gaziresim/ok.gif" width="13" height="9" /><span class="yazi"> <font color="#CC0000">
			  <%
		...
		sql="SELECT t.id, t.gonderen, t.ek, t.fakulte, t.tarih, t.baslik, t.icerik, t.populasyon, t.girenid, IIF(ISNULL(ty.yorumsayisi), 0, ty.yorumsayisi) FROM tiraf AS t LEFT JOIN (SELECT tirafid , COUNT(id) AS yorumsayisi FROM Yorum GROUP BY tirafid) AS ty ON t.id=ty.tirafid where t.tarih <= (Date() - Weekday(Date(), 2) + 2) and t.tarih >= (Date() - Weekday(Date(), 2) -5) and fakulte like '%"&qfakulte&"%' ORDER BY ty.yorumsayisi DESC"
		tiraf.open sql,baglanti,1,3
		else
		fakulte=temizle(request.form("fakulte"))
		set tiraf=server.createobject("adodb.recordset")
		sql="SELECT t.id, t.gonderen, t.ek, t.fakulte, t.tarih, t.baslik, t.icerik, t.populasyon, t.girenid, IIF(ISNULL(ty.yorumsayisi), 0, ty.yorumsayisi) FROM tiraf AS t LEFT JOIN (SELECT tirafid , COUNT(id) AS yorumsayisi FROM Yorum GROUP BY tirafid) AS ty ON t.id=ty.tirafid where t.tarih <= (Date() - Weekday(Date(), 2) + 2) and t.tarih >= (Date() - Weekday(Date(), 2) -5) and fakulte like '%"&fakulte&"%' ORDER BY ty.yorumsayisi DESC"
		tiraf.open sql,baglanti,1,3
		end if
