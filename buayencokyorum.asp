...
<head>
<title>Bu Ay En &Ccedil;ok Yorum Alanlar - Gazitiraf.net</title>
      ...
      <tr>
        ...
        <td width="505" valign="top">
		  ...
		  <table width="512" height="19" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td><span class="yazi style3"><strong>Bu Ay En &Ccedil;ok Yorumlananlar</strong></span> <img src="gaziresim/ok.gif" width="13" height="9" /><span class="yazi"> Tüm Fakülteler Gösteriliyor. </span></td>
            </tr>
          </table>
		  <%
		set tiraf = server.createobject("adodb.recordset")
sql="SELECT t.id, t.gonderen, t.ek, t.fakulte, t.tarih, t.baslik, t.icerik, t.populasyon, t.girenid, IIF(ISNULL(ty.yorumsayisi), 0, ty.yorumsayisi) FROM tiraf AS t LEFT JOIN (SELECT tirafid , COUNT(id) AS yorumsayisi FROM Yorum GROUP BY tirafid) AS ty ON t.id=ty.tirafid WHERE t.tarih >= DateSerial(Year(Date()), Month(Date()), 1) and t.tarih <= DateSerial(Year(Date()), Month(Date()) + 1, 0) ORDER BY ty.yorumsayisi DESC"
