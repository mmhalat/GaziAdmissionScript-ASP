...
<title>Bu Hafta En Eskiler - Gazitiraf.net</title>
      ...
      <tr>
        ...
        <td width="505" valign="top">
		  ...
		  <table width="512" height="19" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td><span class="yazi style3"><strong>Bu Hafta En Eskiler</strong></span> <img src="gaziresim/ok.gif" width="13" height="9" /><span class="yazi"><font color="#CC0000"> 
			  <%
			  if not request.QueryString("fakulte")="" then
			 %><% =temizle(request.QueryString("fakulte"))%>
			  <%else%><%
			  =temizle(request.Form("fakulte"))%><%
			  end if%> Fak&uuml;ltesi Gösteriliyor.</font></span></td>
            </tr>
          </table>
          		  <%if not request.QueryString("fakulte")="" then
		  qfakulte = temizle(request.QueryString("fakulte"))
		set tiraf=server.createobject("adodb.recordset")
sql="select * from tiraf where onay=1 and tarih <= (Date() - Weekday(Date(), 2) + 2) and tarih >= (Date() - Weekday(Date(), 2) -5) and fakulte like '%"&qfakulte&"%' order by id asc"
		tiraf.open sql,baglanti,1,3
		else
		fakulte=temizle(request.form("fakulte"))
		set tiraf=server.createobject("adodb.recordset")
sql="select * from tiraf where onay=1 and tarih <= (Date() - Weekday(Date(), 2) + 2) and tarih >= (Date() - Weekday(Date(), 2) -5) and fakulte like '%"&fakulte&"%' order by id asc"
		tiraf.open sql,baglanti,1,3
		end if
		  <%
