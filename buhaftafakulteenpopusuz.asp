...
<title>Bu Hafta En Pop&uuml;lerler - Gazitiraf.net</title>
      ...
      <tr>
        ...
        <td width="505" valign="top">
		  ...
		  <table width="512" height="19" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td><span class="yazi style3"><strong>Bu Hafta En Pop&uuml;s&uuml;zler</strong></span> <img src="gaziresim/ok.gif" width="13" height="9" /><span class="yazi"> <font color="#CC0000">
			  <%
			  if not request.QueryString("fakulte")="" then
			 %><% =temizle(request.QueryString("fakulte"))%>
			  <%else%><%
			  =temizle(request.Form("fakulte"))%><%
			  end if%> Fak&uuml;ltesi Gösteriliyor.</font></span>
              
              </td>
            </tr>
          </table>
          		  <%if not request.QueryString("fakulte")="" then
		  qfakulte = temizle(request.QueryString("fakulte"))
		set tiraf=server.createobject("adodb.recordset")
sql="select * from tiraf where onay=1 and tarih <= (Date() - Weekday(Date(), 2) + 2) and tarih >= (Date() - Weekday(Date(), 2) -5) and fakulte like '%"&qfakulte&"%' order by populasyon asc"
		tiraf.open sql,baglanti,1,3
		else
		fakulte=temizle(request.form("fakulte"))
		set tiraf=server.createobject("adodb.recordset")
sql="select * from tiraf where onay=1 and tarih <= (Date() - Weekday(Date(), 2) + 2) and tarih >= (Date() - Weekday(Date(), 2) -5) and fakulte like '%"&fakulte&"%' order by populasyon asc"
		tiraf.open sql,baglanti,1,3
		end if
		  <%
		...
		If Toplam = 0 Then
		intSayfaSayisi = 0
		Response.Write "Tabloda Kayit Yok"
		Else
...
<a href="?sayfa=1&fakulte=<%=temizle(request.form("fakulte"))%>" title="Ýlk Sayfa">[««]</a>
<a href="?sayfa=<%=intGecerliSayfa-1%>&fakulte=<%=temizle(request.form("fakulte"))%>" title="Önceki Sayfa">[«]</a><%
End If
%>
<%
...
Response.Write " <b>"
Else %>
<a href="?sayfa=<%=i%>&fakulte=<%=temizle(request.form("fakulte"))%>" title="<%=i%>. Sayfa">
<%
End If
...
<a href="?sayfa=<%=intGecerliSayfa+1%>&fakulte=<%=temizle(request.form("fakulte"))%>" title="Sonraki Sayfa">[»]</a>
<a href="?sayfa=<%=intSayfaSayisi%>&fakulte=<%=temizle(request.form("fakulte"))%>" title="Son Sayfa">[»»]</a><%
End If
%>
		  ...
		  <table width="190" height="464" border="0" align="left" cellpadding="0" cellspacing="0">
            ...
            <tr>
              <td colspan="2" valign="top"><div align="center" class="yazi"><strong>Fakülte Seç :</strong>
                 <FORM action="buhaftafakultesec.asp" method="post">
            ...
            <tr>
              ...
              <td width="202" valign="top"><div align="center" class="yazi">
                <div align="left">               
<form id="fakulte" method="post" action="buhaftafakulteenpopuler.asp" name="fakulte">
...
En Populerler</a>  
            ...
            <tr>
              ...
              <td valign="top"><div align="center" class="yazi">
                ...
                <form id="fakulte" method="post" action="buhaftafakulteeneskiler.asp" name="eneski">
                <input type="hidden" name="fakulte" value="<%=request.form("fakulte")%>" />
            ...
            <tr>
              ...
              <td height="15" valign="top"><div align="center" class="yazi">
                ...
                <form id="fakulte" method="post" action="buhaftafakulteenpopusuz.asp" name="enpopusuz">
                <input type="hidden" name="fakulte" value="<%=request.form("fakulte")%>" />
            ...
            <tr>
              ...
              <td height="15" valign="top"><div align="center" class="yazi">
                ...
                <form id="fakulte" method="post" action="buhaftafakulteencokyorum.asp" name="encokyorum">
                <input type="hidden" name="fakulte" value="<%=request.form("fakulte")%>" />
